...
const char *typeofdata = "v18_PFmuon/20240402/Reco_v18c_694_ov10_notrkW";
   ...
   string HistName="HPhi", string XTitleName = "#Delta#phi_{Z,track}", string YTitleName = "dN/d#Delta#phi", int rebinnum=1)
{
   ...
   TNamed *npM_tN = (TNamed *) file_ppMC->Get(Form("%s/%s",FolderName.c_str(),NameEntryCount.c_str()));
...
int main(int argc, char *argv[]){
   ...
   if(isgen)
      file_sigMC = TFile::Open("~/eos_base/BasicPlots/GraphMCSignal_v18c_PFmuon_notrkW.root","read");
   else
      file_sigMC = TFile::Open("~/eos_base/BasicPlots/GraphMCSignal_v18c_PFmuon_notrkW.root","read");
   ...
   if(selfmix)
      ...
      if(isgen)
         file_bkgMC = TFile::Open("~/eos_base/BasicPlots/GraphMCBackground_v18c_694_ov10_notrkW.root","read");
      else
         file_bkgMC = TFile::Open("~/eos_base/BasicPlots/GraphMCBackground_v18c_694_ov10_notrkW.root","read");
   }

   if(isgen)
      file_ppMC  = TFile::Open("~/eos_base/BasicPlots/GraphPPMC0NPU_v18c_notrkW.root","read");
   else
      file_ppMC  = TFile::Open("~/eos_base/BasicPlots/GraphPPMC0NPU_v18c_notrkW.root","read");
